...
class board:
    ...
    def move(self, color, type, start, dest):
        ...
        if (dest_x, dest_y) not in active_piece.get_valid_squares(self.get_graph()):
            return "Illegal move"
    ...
    def can_move(self, color):
        ...
        for p in {piece for piece in self.all_pieces if piece.get_color() == color}:
            ...
            type = p.get_type()
            for s in p.get_valid_squares(self.get_graph()):
                err = self.move(color, type, start, chr(s[0] + 97) + str(s[1] + 1))
                f.write(color + "\t" + type + "\t" + start + "\t" + chr(s[0] + 97) + str(s[1] + 1) + "\tERR: " + str(err) + "\n")
